...
def build_report(dataset: Path, runs: list[dict], report: Path,
                 ...
                 title: str = "ActNet Eval", split: str = "test",
                 sample_id_globs: list[str] | None = None,
                 sample_id_regexes: list[str] | None = None) -> Path:
    compiled_id_regexes = [re.compile(pattern) for pattern in sample_id_regexes or []]
    records = {}
    with dataset.open(encoding="utf-8") as handle:
        for line in handle:
            record = json.loads(line)
            if split == "all" or record["split"] == split:
                matches_glob = any(fnmatch.fnmatchcase(record["sample_id"], pattern)
                                   for pattern in sample_id_globs or [])
                matches_regex = any(pattern.fullmatch(record["sample_id"])
                                    for pattern in compiled_id_regexes)
                if (sample_id_globs or compiled_id_regexes) and not (
        ...
        for experiment in _group_runs(runs, _experiment_key):
            ...
            for variants in _group_runs(available, _generation_key):
                ...
                generation_groups.append(
                    ...
                    f'<div class="shared-gantt"><div aria-hidden="true"></div><div>'
                    f'{_visibility_gantt(record, run["prompt"], run["model"])}</div>'
                    f'</div></div>'
                )
            run = available[0]
            results.append(
