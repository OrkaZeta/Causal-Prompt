...
def load_transformer(checkpoint_path: str, cfg: TrainConfig, device: torch.device, dtype: torch.dtype) -> LTX23Model:
    ...
    resume = Path(cfg.paths.resume_checkpoint or "") / "transformer.pt"
    if resume.is_file():
        with torch.device("meta"):
            model = LTX23Model(**filtered)
        state = torch.load(resume, map_location="cpu", weights_only=True, mmap=True)
        expected = set(model.state_dict())
        unused_action = [k for k in state if k not in expected and "action_adaln" in k]
        for key in unused_action:
            del state[key]
        if unused_action:
            print(f"[WM-AlayaWorld] unused action-only tensors: {unused_action}", flush=True)
        model.load_state_dict(state, strict=True, assign=True)
        print(f"[WM-AlayaWorld] AR base strict load: {len(state)} tensors, missing=0 unexpected=0", flush=True)
...
def load_vae(
    ...
    if state_dict is None and checkpoint_path and Path(checkpoint_path).exists():
        with safetensors.safe_open(checkpoint_path, framework="pt", device="cpu") as handle:
            state_dict = {k: handle.get_tensor(k) for k in handle.keys() if k.startswith("vae.")}
...
def load_text_encoder(
    ...
    if state_dict is None:  # da3: shared merged state when provided; else read here (vigeo path)
        with safetensors.safe_open(checkpoint_path, framework="pt", device="cpu") as handle:
            state_dict = {k: handle.get_tensor(k) for k in handle.keys()
                          if k.startswith("text_embedding_projection.") or "video_embeddings_connector" in k}
    fe = {
