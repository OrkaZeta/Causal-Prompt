...
class CausalInferencePipeline(torch.nn.Module):
    def __init__(
        ...
        model_kwargs["model_path"] = base_model_path
        self.generator = WanDiffusionWrapper(
            **model_kwargs, is_causal=True) if generator is None else generator
        self.text_encoder = WanTextEncoder(base_model_path) if text_encoder is None else text_encoder
        self.vae = WanVAEWrapper(base_model_path) if vae is None else vae
    ...
    def inference(
        ...
        block_text_prompts: Optional[List[str] | List[List[str]]] = None,
        ...
        if block_text_prompts is not None:
            if batch_size == 1 and block_text_prompts and isinstance(block_text_prompts[0], str):
                block_text_prompts = [block_text_prompts]
            if len(block_text_prompts) != batch_size or any(
                len(prompts) != len(all_num_frames) for prompts in block_text_prompts
            ):
                raise ValueError(
                    "Expected one scheduled prompt per sample and generated block."
                )

        encoded_prompts = {}
        active_prompt = None

        def conditioning_for(block_index: int):
            nonlocal active_prompt
            prompts = (
                text_prompts
                if block_text_prompts is None
                else [prompts[block_index] for prompts in block_text_prompts]
            )
            key = tuple(prompts)
            if key not in encoded_prompts:
                encoded_prompts[key] = self.text_encoder(text_prompts=prompts)
            if active_prompt is not None and key != active_prompt:
                for cache in self.crossattn_cache:
                    cache["is_init"] = False
            active_prompt = key
            return encoded_prompts[key]

        conditional_dict = conditioning_for(0)
        ...
        for block_index, current_num_frames in enumerate(tqdm.tqdm(all_num_frames)):
            conditional_dict = conditioning_for(block_index)
