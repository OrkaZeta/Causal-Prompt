...
class SelfForcingTrainingPipeline:
    ...
    def inference_with_trajectory(
            ...
            block_conditional_dicts: Optional[List[dict]] = None,
            **conditional_dict
    ) -> torch.Tensor:
        ...
        if block_conditional_dicts is not None and len(block_conditional_dicts) != len(all_num_frames):
            raise ValueError(
                "Expected one conditioning dictionary per generated block: "
                f"got {len(block_conditional_dicts)}, expected {len(all_num_frames)}."
            )
        ...
        for block_index, current_num_frames in enumerate(all_num_frames):
            ...
            block_conditional_dict = (
                conditional_dict
                if block_conditional_dicts is None
                else block_conditional_dicts[block_index]
            )
            if block_index > 0 and block_conditional_dicts is not None:
                if block_conditional_dict is not block_conditional_dicts[block_index - 1]:
