...
class WanTextEncoder(torch.nn.Module):
    def __init__(self, checkpoint_dir: str) -> None:
        ...
        self.text_encoder.load_state_dict(
            torch.load(os.path.join(checkpoint_dir, "models_t5_umt5-xxl-enc-bf16.pth"),
                       map_location='cpu', weights_only=False)
        )

        self.tokenizer = HuggingfaceTokenizer(
            name=os.path.join(checkpoint_dir, "google", "umt5-xxl"),
            seq_len=512,
            clean='whitespace',
        )
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self, checkpoint_dir: str):
        ...
        self.model = _video_vae(
            pretrained_path=os.path.join(checkpoint_dir, "Wan2.1_VAE.pth"),
            z_dim=16,
        ).eval().requires_grad_(False)
...
class WanDiffusionWrapper(torch.nn.Module):
    def __init__(
        ...
        model_path = model_path or os.path.join("wan_models", model_name)
        if is_causal:
