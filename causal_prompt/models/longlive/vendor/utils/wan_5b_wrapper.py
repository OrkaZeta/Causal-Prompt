...
def _assets():
    return Path(os.environ["LONGLIVE_BASE_MODEL_PATH"])
...
def _tokenizer():
    ...
    for path in (root / "google/umt5-xxl", root / "umt5-xxl"):
        if path.is_dir():
            return str(path)
    raise FileNotFoundError(f"No UMT5 tokenizer under {root}")
...
class WanTextEncoder(torch.nn.Module):
    def __init__(self) -> None:
        ...
        self.text_encoder.load_state_dict(
            torch.load(str(_assets() / "models_t5_umt5-xxl-enc-bf16.pth"),
        ...
        self.tokenizer = HuggingfaceTokenizer(
            name=_tokenizer(), seq_len=512, clean='whitespace')
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self):
        ...
        self.model = _video_vae(
            pretrained_path=str(_assets() / "Wan2.2_VAE.pth"),
        ).eval().requires_grad_(False)
    ...
    def __init__(
        ...
        if is_causal:
            self.model = CausalWanModel.from_pretrained(
                str(_assets()), local_attn_size=local_attn_size, sink_size=sink_size,
                num_frame_per_block=num_frame_per_block)
        else:
            self.model = WanModel.from_pretrained(str(_assets()))
        self.model.eval()
